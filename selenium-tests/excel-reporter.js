...
module.exports = {
  ...
  generateAndPrint: async () => {
    ...
    console.log('\n======================================');
    console.log('         E2E SELENIUM TEST REPORT      ');
    console.log('======================================');
    ...
    // Also write copy directly to website's public reports folder
    const webReportsDir = path.join(__dirname, '..', 'website', 'public', 'reports');
    try {
      fs.mkdirSync(webReportsDir, { recursive: true });
      fs.writeFileSync(path.join(webReportsDir, 'test-report.json'), fs.readFileSync(reportPath));
    ...
    const sheet = workbook.addWorksheet('E2E Web Test Report', {
    ...
    titleCell.value = 'SmartCanteen E2E Web Test Execution Report';
    ...
    // 2. Summary stats cards (Headers and Values)
    // We will build a neat grid for: Total, Passed, Failed, Pass Rate, Duration
    const statCards = [
    ...
    const headers = [
      ...
      { col: 'D', text: 'Test Description', width: 35 },
    ...
    results.forEach(r => {
      ...
      if (r.passed) {
        ...
        rStatus.font = { name: 'Arial', size: 10, bold: true, color: { argb: 'B91C1C' } }; // Red
        rStatus.fill = {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: 'FEE2E2' }
    ...
    const excelPath = path.join(__dirname, 'selenium-report.xlsx');
    await workbook.xlsx.writeFile(excelPath);
    console.log(`Excel report created successfully at: ${excelPath}`);

    // Copy Excel directly to website's public reports folder
    try {
      const webExcelPath = path.join(webReportsDir, 'selenium-report.xlsx');
